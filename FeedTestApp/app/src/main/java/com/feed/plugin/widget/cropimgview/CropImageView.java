package com.feed.plugin.widget.cropimgview;
...
import com.feed.plugin.widget.cropimgview.animation.SimpleValueAnimator;
import com.feed.plugin.widget.cropimgview.animation.SimpleValueAnimatorListener;
import com.feed.plugin.widget.cropimgview.animation.ValueAnimatorV14;
import com.feed.plugin.widget.cropimgview.animation.ValueAnimatorV8;
import com.feed.plugin.widget.cropimgview.callback.Callback;
...
public class CropImageView extends ImageView{
    ...
    private static final int POINTER_DRAG = 2;
            ...
            case MotionEvent.ACTION_MOVE:
                if(mode == DRAG){
                    ...
                    if(event.getPointerCount() == 2)
                    {
                        ...
                        float distValue = newDist - oldDist;

                        oldDist = newDist;

                        moveCropArea(distValue, distValue);
                        invalidate();
                    }
...
//                break;
    ...
    private void moveCropArea(float diffX, float diffY){
        float dx = diffX / 2;
        float dy = (diffX * getRatioY() / getRatioX()) / 2;
        mFrameRect.left -= dx;
        mFrameRect.top -= dy;
        mFrameRect.right += dx;
        mFrameRect.bottom += dy;
        ...
        float ox, oy;
        if((mFrameRect.right >= mImageRect.right &&  mFrameRect.top <= mImageRect.top) ||
                (mFrameRect.left <= mImageRect.left &&  mFrameRect.bottom >= mImageRect.bottom))
...
//        }
    ...
    private Uri saveImage(Bitmap bitmap, final Uri uri) throws IOException, IllegalStateException{
        ...
        try{
            ...
            CropUtils.copyExifInfo(getContext(), mSourceUri, uri, bitmap.getWidth(), bitmap.getHeight());
            CropUtils.updateGalleryInfo(getContext(), uri);
            return uri;
        }finally{
            CropUtils.closeQuietly(outputStream);
        }
    }
