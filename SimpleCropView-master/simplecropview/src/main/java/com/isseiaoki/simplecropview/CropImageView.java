        ...
        switch(event.getAction() & MotionEvent.ACTION_MASK)
        {
            ...
            case MotionEvent.ACTION_POINTER_DOWN:
                if(mode == DRAG){    // 모드가 drag였던 것이면 ZOOM으로한다.
                    ...
                    oldDist = (float) Math.sqrt((start.x - event.getX(1)) * (start.x - event.getX(1)) +
                            (start.y - event.getY(1)) * (start.y - event.getY(1)));
                }
                return true;
            ...
            case MotionEvent.ACTION_MOVE:
                if(mode == DRAG){
                    ...
                    int dx = 0, dy = 0;
                    double dist = 0.0f;
                    dx = (int) ((event.getX(1) - event.getX(0)));
                    dy = (int) ((event.getY(1) - event.getY(0)));
                    dist = Math.sqrt((dx * dx + dy * dy));
                    double div = dist / oldDist;
                    if(div < 0.5)    // 너무 작아지거나 커지는 것을 방지.
                        div = 0.5;
                    else if(div > 2.0)
                        div = 2.0;
                    ...
                    moveCropArea((float)div, (float)div);
